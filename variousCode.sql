...
--search field names in tables
  SELECT t.name AS table_name, SCHEMA_NAME(schema_id) AS schema_name, c.name AS column_name FROM sys.tables AS t INNER JOIN sys.columns c ON t.OBJECT_ID = c.OBJECT_ID WHERE c.name LIKE '%Class ID%' ORDER BY schema_name, table_name; 
...
select DBPrincipal_2.nam[SBPG\etolle]e as role, DBPrincipal_1.name as owner 
...
sp_spaceused 'CENSUS_HOUSE_ALASKA_SIMPLE'
...
EXEC  sp_databases
-- STFID 15 varchar
CONVERT(VARCHAR(15), CONVERT(DECIMAL(18,0), X.STFID))AS STFID,

--Does Table Exist
  if exists (select 1 from INFORMATION_SCHEMA.TABLES where TABLE_NAME = 'NJDataImport_Date_Geocoded_SJ')
    print 'Exists'
...
insert into [GIS_Test].[dbo].[AuctionList] VALUES ('BER-400085' , '1')
...
SELECT * into [NJ_RAD_DATA].[temp].[NJDataImport_Geocoded_for_SQL082] FROM 
...
--copy a table
SELECT * INTO newTable FROM vwCurrent_Polling_Places

--datetime   http://www.sqlusa.com/bestpractices/datetimeconversion/
  convert(datetime, STUFF(STUFF(convert (bigint,[DateValue]),5,0,'-'),8,0,'-') + ' ' +  [DepartTime],121 )
...
ADD ( 	Address_Error 	varchar(150),
  		GeoAddress 			varchar(150)
  		City 			varchar(150)
  		State 			varchar(150)
  		Zip 			varchar(5)
  		Zip9 			varchar(10) );
...
SELECT
	Distinct JoinField from p0106

INSERT INTO PostalMaster
SELECT
	Distinct JoinField from p0705
WHERE 
	Joinfield not in (select joinfield from postalmaster)
...
--------Query Table Names	
	select * from INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA = 'dbo' and TABLE_NAME like '%_Pin%'
order by TABLE_NAME
...
if exists (select name from sysindexes where name = 'sidx_Shape_CensusByBlock')
       begin
...
SET @MinX = (select MIN(shape.STEnvelope().STPointN(1).STX) from [Private.Data].[CensusByBlock])
SET @MinY = (select MIN(shape.STEnvelope().STPointN(1).STY) from [Private.Data].[CensusByBlock])
SET @MaxX = (select MAX(shape.STEnvelope().STPointN(3).STX) from [Private.Data].[CensusByBlock])
SET @MaxY = (select MAX(shape.STEnvelope().STPointN(3).STY) from [Private.Data].[CensusByBlock])
...
declare @strSQL nvarchar(500),@MinX as varchar(10), @MinY as varchar(10), @MaxX as varchar(10), @MaxY as varchar(10)
SET @MinX = (select MIN(Shape.STX) from [Private].[gnis_ptsintrest])
SET @MinY = (select MIN(Shape.STY) from [Private].[gnis_ptsintrest])
SET @MaxX = (select MAX(Shape.STX) from [Private].[gnis_ptsintrest])
SET @MaxY = (select MAX(Shape.STY) from [Private].[gnis_ptsintrest])
...
exec sp_executesql @strSQL
 ...
 MS Excel Function for calculating percentiles.
 =IF(B3 < PERCENTILE.INC(B:B,0.1), 1,IF(AND(B3<PERCENTILE.INC(B:B,0.2),B3>PERCENTILE.INC(B:B,0.1)),2,IF(AND(B3<PERCENTILE.INC(B:B,0.3),B3>PERCENTILE.INC(B:B,0.2)),3,IF(AND(B3<PERCENTILE.INC(B:B,0.4),B3>PERCENTILE.INC(B:B,0.3)),4,IF(AND(B3<PERCENTILE.INC(B:B,0.5),B3>PERCENTILE.INC(B:B,0.4)),5,IF(AND(B3<PERCENTILE.INC(B:B,0.6),B3>PERCENTILE.INC(B:B,0.5)),6,IF(AND(B3<PERCENTILE.INC(B:B,0.7),B3>PERCENTILE.INC(B:B,0.6)),7,IF(AND(B3<PERCENTILE.INC(B:B,0.8),B3>PERCENTILE.INC(B:B,0.7)),8,IF(AND(B3<PERCENTILE.INC(B:B,0.9),B3>PERCENTILE.INC(B:B,0.8)),9,IF(AND(B3<PERCENTILE.INC(B:B,1.0),B3>PERCENTILE.INC(B:B,0.9)),10,0))))))))))
...
SET
  ...
  SELECT  [shape].STAsText() as WKT  from [gis].[dbo].[PRBusinessHooversData]
SELECT  [shape].ToString() as WKT  from [gis].[dbo].[PRBusinessHooversData]
SELECT  [shape].STY as WKT  from [gis].[dbo].[PRBusinessHooversData]

  select shape.AsTextZM() FROM [MEM].[dbo].[PART77SURFACE]
    select shape.STAsText() FROM [MEM].[dbo].[PART77SURFACE]
	  select shape.ToString() FROM [MEM].[dbo].[PART77SURFACE]
POLYGON ((-89.982999999999947 35.082517920000157 470.60000000000582, -89.978000000000009 35.055119750000244 270.60000000000582, -89.974999999999966 35.055167470000072 270.60000000000582, -89.96999999999997 35.082708640000135 470.60000000000582, -89.982999999999947 35.082517920000157 470.60000000000582))
POLYGON ((-89.982999999999947 35.082517920000157, -89.978000000000009 35.055119750000244, -89.974999999999966 35.055167470000072, -89.96999999999997 35.082708640000135, -89.982999999999947 35.082517920000157))
POLYGON ((-89.982999999999947 35.082517920000157 470.60000000000582, -89.978000000000009 35.055119750000244 270.60000000000582, -89.974999999999966 35.055167470000072 270.60000000000582, -89.96999999999997 35.082708640000135 470.60000000000582, -89.982999999999947 35.082517920000157 470.60000000000582))
...
DBCC SHRINKFILE ([excelTest]_Log, 1);
GO
...
geometry::EnvelopeAggregate(Shape).STPointN(3).STY AS MaxX FROM trash.rb_AL092012INITIALRADII


--ShapeValid 
select 	  [Shape].IsValidDetailed() from ____ -- for SQL2012
select 	  [Shape].IsValid() from ____ 

--Label Point
select  shape.STPointOnSurface().ToString()   from [test03].[dbo].[PARCELS]
  --Spatial Join on centroid 
  on C.Shape.STIntersects (P.Shape.STPointOnSurface()) = 1 and DISTRICT = 'A'
...
-- geometry types
   SELECT 'Polygon' AS gtype, count (*), [ShapeValid].STGeometryType() from [test].[temp2].[ClaimGeometries_20140929_Special_Version2] group by [ShapeValid].STGeometryType()
...
WHERE        (OGC.Status <> 'U') --and  LHCGeocodesComplete.[Loc_name] not in ('US_RoofTop', 'PointAddress')
...
select count (*) ,[Loc_name] from [dbo].[LHCGeocodesComplete]
group by [Loc_name]
...
insert into LHCGeocodesComplete select * from [dbo].[LHCGeocodes20121023]
...
STU.Shape.STDistance(SCH.Shape)*100/1.60934 as Distance'

--Changing Users on database
exec sp_change_users_login 'UPDATE_ONE', 'sqluser', 'sqluser' -- change sqluser to desired user

--ACS DataImports
select  * INTO e20115de0103000  FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0','Text;database=C:\GIS\data\national\census\DE\Delaware_Tracts_Block_Groups_Only;HDR=No;FORMAT=Delimited(,)', 'SELECT * FROM [e20115de0103000.txt] ') AliasOfTxtFile
...
--Excel Files
ALTER view [dbo].[VWpplaces] as SELECT * FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0','EXCEL 12.0;Database=C:\GIS\Projects\Elections\Pres_Poll_Places_Results.xlsx;HDR=YES;IMEX=1','SELECT * FROM [Sheet1$]');


--DBF Files
SELECT *FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0','dBase IV;Database=C:\GIS\Projects\GCR\Mona\PLD','SELECT * FROM [PLDTaxes]') AliasOfDBF

--TextFile
select * from openrowset('Microsoft.ACE.OLEDB.12.0','text;Database=C:\TEMP;HDR=Yes;Format=Delimited(,)','select * from [DC12.csv]') AliasOfTxtFile --or
select * from openrowset('Microsoft.ACE.OLEDB.12.0','text;Database=C:\TEMP','select * from [DC12.csv]') AliasOfTxtFile
select * INTO BridgeInventory2012AK12 from openrowset('Microsoft.ACE.OLEDB.12.0','text;Database=C:\Users\rpoche\Google Drive\gcr\gis\2012NationalBridgeInventory','select * from AK12.txt ')AliasOfDBF

--Works Great 
select  * INTO temp  FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0','Text;Database=C:\GIS\data\national\census\ACS2011;HDR=No;FORMAT=Delimited(,)', 'SELECT * FROM [g20115la.csv] ') AliasOfTxtFile
--All into one Row
select  * INTO [dbo].g20095la3BULK  FROM OPENROWSET(BULK N'C:\GIS\Projects\WestJeff\CensusACS\Louisiana_Tracts_Block_Groups_Only2009\g20095la.txt',SINGLE_BLOB) AliasOfTxtFile
--FromExcel
SELECT * FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0', 'Excel 12.0;Database=C:\Users\file name.xslx,[Sheet1$])
...
Format=TabDelimited
Format=Fixed
C:\Users\rpoche\Downloads\zbp11detail
...
INSERT INTO [BPTable1] (types,color,size) VALUES ('golden','yellow\'s my favorite colour','large');
...
INSERT INTO [dbo].[g20115la2] select * from  OPENROWSET('Microsoft.ACE.OLEDB.12.0','Text;Database=C:\GIS\Projects\WestJeff\CensusACS\Louisiana_Tracts_Block_Groups_Only2009\;HDR=No;Format=FixedLength(1270)', 'SELECT * FROM [g20095la.txt]') AliasOfTxtFile
stf314ia
stf310ia
stf328ia
stf330ia
INSERT INTO [dbo].[stf310ia] SELECT * FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0','EXCEL 12.0;Database=\\ws-884bqw1\c$\GIS\projects\23088-WestJeff\Census1990\STF3\stf314ia.xls;HDR=YES;IMEX=1','SELECT * FROM [Sheet1$]');

BULK INSERT g20115la3
   FROM 'C:\GIS\Projects\WestJeff\CensusACS\Louisiana_Tracts_Block_Groups_Only2009\g20095la.txt';
...
="Insert into SignCondition values ("&A5&",'"&B5&"',"&"'"&C5&"')" --For Inserting Values
...
Standard Deviation or calculating values
  with data as 
  (
  select 1 as num
  union all 
...
DECLARE @GeocodeTable TABLE( geocode ESRIGEocodeResult)
INSERT INTO
	@GeocodeTable
SELECT
...
DECLARE @GeocodeTable TABLE( ogr_fid int,  geocode ESRIGEocodeResult)
